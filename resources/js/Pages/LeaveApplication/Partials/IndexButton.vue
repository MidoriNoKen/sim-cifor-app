...
const unrejectBySupervisor = (leaveApplication) => {
    Inertia.post(
        `/leave-applications/${leaveApplication.id}/unreject`
    );
};
...
<template>
    ...
    <div v-if="leaveApplication.isSupervisor">
        ...
        ">
            <button @click="
                ...
                " class="btn btn-danger hover-background btn-sm m-1" style="color: white">
                Disapprove
            </button>
            <button class="btn btn-sm m-1"
                style="background-color: gray; color: white; opacity: 0.5; cursor: not-allowed;" disabled>
                Reject
            </button>
        </span>
        ...
        <span v-else-if="leaveApplication.status === 'Rejected by Manager'">
            <button class="btn btn-danger hover-background btn-sm m-1" style="color: white" disabled>
                Manager Rejected
            </button>
            <button class="btn btn-danger hover-background btn-sm m-1" style="color: white" disabled>
                Manager Rejected
    ...
    <div v-else-if="leaveApplication.isManager">
        ...
        <span v-else-if="leaveApplication.status === 'Rejected by Supervisor'">
            ...
            <button class="btn btn-danger hover-background btn-sm m-1" style="color: white">
                Supervisor Rejected
            </button>
        </span>
        <span v-else-if="leaveApplication.status === 'Rejected by Manager'">
